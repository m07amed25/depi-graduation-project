import {
  Body,
  Container,
  Head,
  Heading,
  Html,
  Img,
  Preview,
  Section,
  Text,
  Tailwind,
  Hr,
} from "@react-email/components";
import { render } from "@react-email/render";
import type { AdminDemotedEmailParams } from "@/types/email";

const baseUrl = process.env.APP_URL || "http://localhost:3000";

export function AdminDemotedEmail({
  params,
}: {
  params: AdminDemotedEmailParams;
}) {
  const { userName, demotedByName } = params;
  const previewText =
    "Your Administrator privileges have been revoked - Code Catch";

  return (
    <Html>
      <Tailwind>
        <Head />
        <Preview>{previewText}</Preview>
        <Body className="bg-gray-100 font-sans p-4">
          <Container className="mx-auto my-10 max-w-[600px] rounded-lg bg-white p-8 shadow-sm">
            {/* Logo Section */}
            <Section className="mb-6">
              <Img
                src={`${baseUrl}/file.svg`}
                width="40"
                height="40"
                alt="Code Catch"
                className="mx-auto"
              />
            </Section>

            {/* Header */}
            <Heading className="mb-6 text-2xl font-bold text-gray-900 text-center">
              Account Status Updated
            </Heading>

            <Text className="mb-4 text-base leading-7 text-gray-700">
              Hello {userName || "there"},
            </Text>

            <Text className="mb-6 text-base leading-7 text-gray-700">
              Your **Administrator** privileges on the **Code Catch**
              platform have been revoked by **{demotedByName}**.
            </Text>

            <Section className="mb-8 p-6 bg-amber-50 rounded-xl border border-amber-100">
              <Text className="m-0 text-base font-semibold text-amber-900">
                What changed?
              </Text>
              <ul className="mt-4 text-sm text-amber-800 leading-6 list-disc pl-5">
                <li>You no longer have access to the Admin Panel</li>
                <li>You can still use the platform as a regular user</li>
                <li>
                  Your existing repositories and reviews remain unaffected
                </li>
              </ul>
            </Section>

            <Text className="mb-8 text-base leading-7 text-gray-700">
              If you have any questions regarding this change, please contact
              the system owner.
            </Text>

            <Hr className="my-8 border-gray-200" />

            <Text className="text-sm text-gray-400 text-center">
              This is an automated notification.
              <br />
              &copy; {new Date().getFullYear()} Code Catch. All rights
              reserved.
            </Text>
          </Container>
        </Body>
      </Tailwind>
    </Html>
  );
}

export async function renderAdminDemotedEmail(
  params: AdminDemotedEmailParams,
): Promise<string> {
  return render(<AdminDemotedEmail params={params} />);
}
